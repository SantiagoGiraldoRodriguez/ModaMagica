...
const crearPreferencia = async (req, res) => {
  ...
  try {
    const pedResult = await pool.query('SELECT * FROM pedido WHERE id_pedido = $1', [id_pedido]);
    ...
    await pool.query('UPDATE pedido SET preference_id = $1 WHERE id_pedido = $2', [response.id, id_pedido]);
...
const webhook = async (req, res) => {
  ...
  try {
    ...
    await pool.query(`
      UPDATE pedido
      SET estado_pago = $1, metodo_pago = $2, referencia_pago = $3, estado_pedido = $4
      WHERE id_pedido = $5
    ...
    if (estado === 'approved') {
      try {
        ...
        const pedRes = await pool.query(`
          SELECT p.id_pedido, p.total_final, u.correo_electronico, u.primer_nombre, u.primer_apellido
          FROM pedido p
        ...
        const detallesRes = await pool.query(`
          SELECT pr.nombre_producto, c.nombre_color, t.nombre_talla, dp.cantidad, dp.precio_vendido
          FROM detalle_pedido dp
          JOIN producto_color pc ON pc.id_producto_color = dp.id_producto_color
          JOIN producto pr ON pr.id_producto = pc.id_producto
        ...
        if (pedRes.rows.length > 0) {
          ...
          const filas = detallesRes.rows.map(d => `
            <tr>
              ...
              <td style="padding:8px;border-bottom:1px solid #f0f0f0;text-align:right;">$${Number(d.precio_vendido).toLocaleString('es-CO')}</td>
            </tr>
          `).join('');
          ...
          const html = emailBase(`
            ...
            <p style="text-align:right;font-size:18px;"><strong>Total: $${Number(total_final).toLocaleString('es-CO')}</strong></p>
