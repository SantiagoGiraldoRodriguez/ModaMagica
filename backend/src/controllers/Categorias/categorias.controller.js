...
const remove = async (req, res) => {
  try {
    const productos = await pool.query(
      'SELECT id_producto FROM producto_categoria WHERE id_categoria = $1 LIMIT 1',
      [req.params.id]
    );
    if (productos.rows.length > 0)
      return res.status(409).json({ error: 'No se puede eliminar la categoria porque tiene un producto asociado. Solo se pueden eliminar la categoria sin productos.' });
