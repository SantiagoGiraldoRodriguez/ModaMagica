...
// ═════════════════════════════════════════════
// ═════════════════════════════════════════════
// GET ALL — VERSIÓN TIENDA (público)
// Igual que getAll, pero descuenta del stock de cada talla las
// reservas temporales activas de OTRAS sesiones de carrito, para
// que dos clientes no puedan "comprar" al mismo tiempo más unidades
// de las que realmente quedan disponibles.
// ═════════════════════════════════════════════
const getAllTienda = async (req, res) => {
  ...
  try {
    ...
    // Limpieza de reservas vencidas antes de calcular disponibilidad
    await pool.query(`DELETE FROM reserva_stock WHERE expira_en < now()`);

    const result = await pool.query(`
      SELECT
        p.*,
        COALESCE(ic_sum.stock_total, 0)
          AS stock_total
      FROM producto p
      ...
      LEFT JOIN (
        ...
        JOIN inventario_color ic
          ON ic.id_producto_color =
             pc.id_producto_color

        GROUP BY pc.id_producto
      ) ic_sum
        ON ic_sum.id_producto = p.id_producto

      ORDER BY p.id_producto ASC
    `);

    // Reservas activas agrupadas por variante (id_producto_color + id_talla),
    // excluyendo las de la propia sesión del cliente que está consultando.
    ...
    const productos = await Promise.all(
      ...
      result.rows.map(async p => {
        ...
        const [colores, categorias, imagenes] =
          await Promise.all([
        ...
        // Ajustar stock_actual de cada talla restando lo reservado por otros
...
const create = async (req, res) => {
  ...
  const categoriasArr =
    resolverCategorias(req.body);

  const errores =
    validar(req.body, categoriasArr);
  ...
  if (
    ...
    req.body.variantes?.length > 0
  ) {
    colores = variantesAColores(
      req.body.variantes
    );
  }
  ...
  try {
    ...
    if (colores && colores.length > 0) {
      ...
      for (const c of colores) {
        ...
        const id_producto_color =
          pc.rows[0].id_producto_color;

        const stockTotal =
          c.tallas?.reduce(
            (a, t) =>
              a + Number(t.stock_actual),
            0
          ) || 0;

        await client.query(`
          INSERT INTO inventario_color (
            id_producto_color,
            stock_actual
          )
        ...
        if (c.tallas?.length > 0) {
          ...
          for (const t of c.tallas) {
            ...
            await client.query(`
              INSERT INTO inventario_color_talla (
                id_producto_color,
                id_talla,
                stock_actual
              )
...
const update = async (req, res) => {
  ...
  try {
    ...
    const existe = await client.query(`
      SELECT id_producto
      FROM producto
      WHERE LOWER(nombre_producto) = LOWER($1)
      AND id_producto != $2
    `, [
      nombre_producto.trim(),
      req.params.id
    ...
    if (colores && colores.length > 0) {
      ...
      for (const c of colores) {
        ...
        const pc = await client.query(`
          ...
          ON CONFLICT (id_producto, id_color)
          DO UPDATE SET id_color = EXCLUDED.id_color
...
const remove = async (req, res) => {
  ...
  try {
    ...
    if (pcIds.length > 0) {
      ...
      await client.query(`
        DELETE FROM inventario_color_talla
        WHERE id_producto_color =
        ANY($1::int[])
      `, [pcIds]);
