...
export default function Layout({ children, dark, setDark, collapsed, setCollapsed }) {
  ...
  const menuItems = todosLosMenuItems.filter(item => item.roles.includes(rol))
      ...
      <nav className="navbar">
        ...
        <div className="nav-right">
          ...
          <div className="dropdown">
            <div className="profile-btn" onClick={() => setShowProfile(v => !v)}>
              <div className="avatar">{inicialUsuario}</div>
              <span className="avatar-name">{nombreUsuario}</span>
            </div>
            {showProfile && (
              <div className="dropdown-menu-custom" style={{ minWidth: 180, zIndex: 1000 }}>
                ...
                <button className="dropdown-item-custom danger" onClick={handleCerrarSesion}>
